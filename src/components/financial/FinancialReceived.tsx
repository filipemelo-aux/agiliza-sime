...
import { Search, CheckCircle2, TrendingUp, X, Undo2, Eye } from "lucide-react";
...
export function FinancialReceived() {
  ...
  const fetchData = async () => {
    ...
    if (error) { toast.error("Erro ao carregar recebimentos"); setLoading(false); return; }
    ...
    const mapped: ReceivedItem[] = (data || []).map((p: any) => ({
      id: p.id,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <SummaryCard icon={CheckCircle2} label="Total recebido" value={formatCurrency(total)} valueColor="green" />
        <SummaryCard icon={TrendingUp} label="Registros" value={String(filtered.length)} />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative">
          ...
          <Input
            placeholder="Buscar por cliente, fatura ou obs..."
            value={search}
            ...
            <table className="w-full text-xs">
              ...
              <tfoot className="bg-muted/30 border-t border-border">
                <tr>
                  ...
                  <td className="px-2 py-2 text-right tabular-nums font-mono font-bold text-green-600">{formatCurrency(total)}</td>
                  <td colSpan={2}></td>
                </tr>
              </tfoot>
            </table>
