...
BEGIN
  ...
  LOOP
    ...
    IF v_only_link THEN
      -- Conta já paga: apenas vincula a movimentação de caixa existente, sem criar novo pagamento
      ...
      WHERE m.origem = 'pagamento_despesa'
        ...
        AND NOT EXISTS (
          SELECT 1 FROM public.bank_reconciliation_item_links l
      ...
      IF v_movement_id IS NULL THEN
        RAISE EXCEPTION 'A conta paga selecionada não possui movimentação de caixa disponível para conciliação';
      END IF;
