...
-- 1) Rename column and FK constraint in freight_contracts
ALTER TABLE public.freight_contracts RENAME COLUMN accounts_payable_id TO expense_id;
ALTER TABLE public.freight_contracts RENAME CONSTRAINT freight_contracts_accounts_payable_id_fkey TO freight_contracts_expense_id_fkey;

-- 2) Update functions that reference accounts_payable_id
...
BEGIN
  ...
  _empresa_id := COALESCE(_establishment_id, _cte.establishment_id, (SELECT id FROM public.fiscal_establishments ORDER BY created_at LIMIT 1));
  ...
  INSERT INTO public.freight_contracts (
    ...
    _cte_id, COALESCE(_establishment_id, _cte.establishment_id), _numero, _data_cte,
...
CREATE OR REPLACE FUNCTION public.update_freight_contract_with_payable(_contract_id uuid, _contratado_id uuid, _contratado_nome text, _contratado_documento text, _contratado_tipo text, _motorista_id uuid, _motorista_nome text, _motorista_cpf text, _vehicle_id uuid, _placa_veiculo text, _veiculo_modelo text, _municipio_origem text, _uf_origem text, _municipio_destino text, _uf_destino text, _natureza_carga text, _peso_kg numeric, _valor_tonelada numeric, _valor_total numeric, _observacoes text, _user_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
