-- Permitir CT-e e Contratos de Frete com valor negativo gerarem previsão/conta a pagar para uso em lote.

-- 1) Recriar create_freight_contract_with_payable: gera expense quando valor != 0
...
BEGIN
  ...
  -- Gera conta a pagar quando valor_total != 0 (negativos permitidos para pagamentos em lote / acertos)
...
-- 2) Recriar update_freight_contract_with_payable: sincroniza expense quando valor != 0
...
BEGIN
  ...
  IF COALESCE(_valor_total, 0) <> 0 THEN
    ...
    END IF;
  ELSE
    -- Valor zero: remove conta a pagar pendente
    IF _expense_id IS NOT NULL THEN
