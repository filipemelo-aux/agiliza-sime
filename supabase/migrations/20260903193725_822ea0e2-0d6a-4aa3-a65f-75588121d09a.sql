...
BEGIN
  ...
  SELECT COALESCE(SUM(valor + COALESCE(desconto, 0)), 0), MAX(data_pagamento)
    INTO v_paid, v_last
  FROM public.expense_payments
  WHERE expense_id = _expense_id;

  SELECT COALESCE(SUM(valor + COALESCE(desconto, 0)), 0) INTO v_unallocated
  FROM public.expense_payments
  ...
  LOOP
    SELECT COALESCE(SUM(valor + COALESCE(desconto, 0)), 0) INTO v_explicit
    FROM public.expense_payments
...
BEGIN
  ...
  -- 1ª passagem: valida e monta o plano de alocação
  ...
  LOOP
    ...
    -- o saldo em aberto é sempre quitado integralmente
    v_exp_ids := v_exp_ids || v_expense_id;
    v_inst_ids := v_inst_ids || v_installment_id;
  ...
  -- Diferença: o extrato é a verdade. Sobra => juros; falta => desconto.
  ...
  -- 2ª passagem: efetiva pagamentos e vínculos
    ...
    INSERT INTO public.expense_payments (
      ...
      'Pagamento via conciliação bancária com rateio'
        || CASE WHEN v_juros > 0 THEN ' (juros de ' || to_char(v_juros, 'FM999999990.00') || ')'
                ...
                ELSE '' END,
      COALESCE(_user_id, auth.uid()), v_juros, v_desconto, v_inst_ids[v_i]
    ) RETURNING id INTO v_payment_id;

    UPDATE public.expense_payments SET expense_id = v_exp_ids[v_i] WHERE id = v_payment_id;
