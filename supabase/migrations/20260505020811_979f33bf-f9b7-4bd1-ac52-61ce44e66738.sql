...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'freight_contracts_accounts_payable_id_fkey'
      AND conrelid = 'public.freight_contracts'::regclass
  ) THEN
    ALTER TABLE public.freight_contracts
      DROP CONSTRAINT freight_contracts_accounts_payable_id_fkey;
  END IF;
END $$;

ALTER TABLE public.freight_contracts
  ADD CONSTRAINT freight_contracts_accounts_payable_id_fkey
  FOREIGN KEY (accounts_payable_id)
  REFERENCES public.expenses(id)
  ON DELETE SET NULL;

CREATE OR REPLACE FUNCTION public.fn_contract_city_from_address(_addr text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT NULLIF(
    btrim(
      COALESCE(
        substring(COALESCE(_addr, '') from '([^,;/\-]+)\s*/\s*[A-Z]{2}\s*$'),
        substring(COALESCE(_addr, '') from '-\s*([^,;/\-]+)\s*$')
      )
    ),
    ''
  )
